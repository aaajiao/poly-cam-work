...
import { discoverScenes } from '../../api/discover/discovery-logic'

describe('discovery endpoint validation', () => {
  it('accepts a normal same-basename GLB+PLY pair', () => {
    const result = discoverScenes({ entries: ['scan-a.glb', 'scan-a.ply'], modelsDir: '', validateFiles: false })
  ...
  it('accepts multiple valid pairs', () => {
    ...
    expect(result.scenes.map((s) => s.id)).toEqual(['scan-a', 'scan-b', 'scan-c'])
    expect(result.errors).toHaveLength(0)
  })

  it('rejects orphan GLB file', () => {
    const result = discoverScenes({ entries: ['orphan.glb'], modelsDir: '', validateFiles: false })
  ...
  it('rejects orphan PLY file', () => {
    const result = discoverScenes({ entries: ['orphan.ply'], modelsDir: '', validateFiles: false })
    expect(result.scenes).toHaveLength(0)
    expect(result.errors).toHaveLength(1)
  ...
  it('rejects duplicate GLB files with exact same basename', () => {
    const result = discoverScenes({ entries: ['scan-a.glb', 'scan-a.GLB', 'scan-a.ply'], modelsDir: '', validateFiles: false })
    expect(result.scenes).toHaveLength(0)
    expect(result.errors).toHaveLength(1)
  ...
  it('rejects duplicate PLY files with exact same basename', () => {
    const result = discoverScenes({ entries: ['scan-a.glb', 'scan-a.ply', 'scan-a.PLY'], modelsDir: '', validateFiles: false })
    expect(result.scenes).toHaveLength(0)
    expect(result.errors).toHaveLength(1)
    expect(result.errors[0].code).toBe('duplicate-basename')
    expect(result.errors[0].basename).toBe('scan-a')
  ...
  it('handles mixed valid and invalid cases', () => {
    ...
    expect(result.errors.map((e) => e.code)).toEqual(['orphan-glb', 'orphan-ply'])
  })
  ...
  it('sorts scenes alphabetically', () => {
    ...
    expect(result.scenes.map((s) => s.id)).toEqual(['apple', 'middle', 'zebra'])
  })
})
...
describe('discovery endpoint malformed-file validation', () => {
  ...
  it('rejects malformed GLB file with invalid header', () => {
    ...
    writeFileSync(path.join(tempDir, 'bad.ply'), 'ply\nformat ascii 1.0\nend_header\n')
  ...
  it('rejects malformed PLY file with invalid header', () => {
    writeFileSync(path.join(tempDir, 'bad.glb'), Buffer.from([0x67, 0x6c, 0x54, 0x46, 0x00, 0x00, 0x00, 0x00]))
  ...
  it('accepts valid GLB+PLY pair with correct headers', () => {
    writeFileSync(path.join(tempDir, 'good.glb'), Buffer.from([0x67, 0x6c, 0x54, 0x46, 0x00, 0x00, 0x00, 0x00]))
    writeFileSync(path.join(tempDir, 'good.ply'), 'ply\nformat ascii 1.0\nend_header\n')
  ...
  it('rejects GLB file that is too small', () => {
    ...
    writeFileSync(path.join(tempDir, 'tiny.ply'), 'ply\nformat ascii 1.0\nend_header\n')
  ...
  it('rejects PLY file that is too small', () => {
    writeFileSync(path.join(tempDir, 'tiny.glb'), Buffer.from([0x67, 0x6c, 0x54, 0x46, 0x00, 0x00, 0x00, 0x00]))
