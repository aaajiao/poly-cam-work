...
import {
	asVec3,
	blobToDataUrl,
	buildSceneDraftPayload,
	collectLocalImageIds,
	dataUrlToBlob,
	hasPendingLocalImages,
	isLocalImage,
	isRemoteImage,
	type LegacyAnnotationImage,
	type LocalDraftAnnotationFileRecord,
	type LocalDraftFileRecord,
	type LocalDraftImageFileRecord,
	normalizeDraft,
	readFileAsText,
...
interface ViewerState {
	...
	updateAnnotationContent: (
		...
		content: Partial<
			Pick<
				Annotation,
				"title" | "description" | "images" | "videoUrl" | "links"
			>
		>,
	) => void;
...
export const useViewerStore = create<ViewerState>()(
	persist(
		(set, get) => ({
			...
			setPendingAnnotationInput: (pendingAnnotationInput) =>
				set({ pendingAnnotationInput }),
			setActiveScene: (id) =>
				set({
					activeSceneId: id,
					measurements: [],
			...
			setToolMode: (toolMode) =>
				set((state) => {
					...
					const panelOpen =
						nextMode === "annotate"
							? true
							: togglingOff && toolMode === "annotate"
								? false
								: state.annotationsPanelOpen;
					return {
						toolMode: nextMode,
			...
			addAnnotation: (a) =>
				set((state) => ({
					...
					sceneMutationVersion: bumpSceneMutationVersion(
			...
			removeAnnotation: (id) => {
				...
				set((state) => ({
					...
					openAnnotationPanelIds: state.openAnnotationPanelIds.filter(
						(panelId) => panelId !== id,
					),
					hoveredAnnotationId:
					...
					sceneMutationVersion: (() => {
						...
						return bumpSceneMutationVersion(
							state.sceneMutationVersion,
							annotation.sceneId,
						);
					})(),
			...
			toggleAnnotationPanel: (id) =>
				set((state) => ({
					...
					hoveredAnnotationId:
						state.openAnnotationPanelIds.includes(id) &&
			...
			setAnnotationsPanelOpen: (annotationsPanelOpen) =>
				set({ annotationsPanelOpen }),
			setSidebarOpen: (sidebarOpen) => set({ sidebarOpen }),
			setPresentationMode: (presentationMode) =>
				set((state) => ({
					presentationMode,
					toolMode: presentationMode ? "orbit" : state.toolMode,
					pendingAnnotationInput: presentationMode
						? null
						: state.pendingAnnotationInput,
				})),
			setCameraControlsEnabled: (cameraControlsEnabled) =>
				set({ cameraControlsEnabled }),
			...
			removeUploadedScene: (id) =>
				set((state) => {
					const removedAnnotations = state.annotations.filter(
						(a) => a.sceneId === id,
					);
					...
					return {
						...
						selectedAnnotationId:
							state.selectedAnnotationId &&
							removedIds.has(state.selectedAnnotationId)
								? null
								: state.selectedAnnotationId,
						openAnnotationPanelIds: state.openAnnotationPanelIds.filter(
						...
						hoveredAnnotationId:
							state.hoveredAnnotationId &&
							removedIds.has(state.hoveredAnnotationId)
								? null
								: state.hoveredAnnotationId,
					};
				}),
			...
			syncPresetScenesToCloud: async () => {
				...
				const publishedById = new Map(
					currentPublishedScenes.map((scene) => [scene.id, scene]),
				);
				const syncedModelsInput: Array<{
					id: string;
					name: string;
					glbUrl: string;
					plyUrl: string;
				...
				for (const scene of presetScenes) {
					...
					if (
						existingPublishedScene &&
						hasValidSceneAssetUrls(existingPublishedScene)
					) {
			...
			loadPublishedVersions: async (sceneId) => {
				try {
					...
					set((state) => {
						const nextPublishedVersionByScene = {
			...
			loadDraft: async (sceneId) => {
				...
				const requestVersion =
					(get().loadRequestVersionByScene[sceneId] ?? 0) + 1;
				...
				try {
					const draft = normalizeDraft(
						sceneId,
						await publishApi.getDraft(sceneId),
					);

					if (
						(get().loadRequestVersionByScene[sceneId] ?? 0) !== requestVersion
					) {
						return;
					}

					if (
						(get().sceneMutationVersion[sceneId] ?? 0) !==
					...
					set((state) => ({
						annotations: replaceSceneAnnotations(
							state.annotations,
							sceneId,
							draft.annotations,
						),
				...
				try {
					const release = normalizeDraft(
						sceneId,
						await publishApi.getRelease(sceneId),
					);
				...
				const applySuccess = (revision: number) => {
					set((nextState) => ({
						...
						draftDirtyByScene: {
							...
							[sceneId]: hasPendingLocalImages(
								sceneAnnotations(nextState.annotations, sceneId),
							),
						},
					}));
				};
				...
				const applyFailure = (error: unknown) => {
					...
					if (status === 409) {
						set({
							...
							draftError:
								"Draft changed while saving. Please try Publish again.",
						});
						return;
					}
			...
			publishDraft: async (sceneId, message) => {
				const localImagesToUpload = sceneAnnotations(
					get().annotations,
					sceneId,
				).flatMap((annotation) =>
					annotation.images.filter(isLocalImage).map((image) => ({
				...
				if (localImagesToUpload.length > 0) {
					...
					try {
						for (const localImage of localImagesToUpload) {
							...
							if (!blob) {
								throw new Error(
									`Local image missing: ${localImage.filename}. Please re-add the image.`,
								);
							}
				...
				const draftSnapshot = buildSceneDraftPayload(
					get().annotations,
					sceneId,
					revision,
				);
				...
				set((state) => ({
					...
					publishedVersionsByScene: {
						...
						[sceneId]: Array.from(
							new Set([
								result.version,
								...(state.publishedVersionsByScene[sceneId] ?? []),
							]),
						).sort((a, b) => b - a),
					},
					draftDirtyByScene: {
			...
			deletePublishedVersion: async (sceneId, version) => {
				const result = await publishApi.deletePublishedVersion(
					sceneId,
					version,
				);
			...
			downloadLocalDraft: async (sceneId) => {
				...
				for (const annotation of sceneDraftAnnotations) {
					...
					for (const image of annotation.images) {
						...
						if (!blob) {
							throw new Error(
								`Local image missing: ${image.filename}. Please re-add it before export.`,
							);
						}
			...
			importLocalDraftFile: async (sceneId, file) => {
				...
				const currentSceneAnnotations = sceneAnnotations(
					get().annotations,
					sceneId,
				);
				...
				for (const sourceAnnotation of parsed.annotations) {
					if (!sourceAnnotation || typeof sourceAnnotation !== "object")
						continue;
					...
					for (const sourceImage of Array.isArray(sourceAnnotation.images)
						...
						const filename =
							typeof sourceImage.filename === "string" &&
							sourceImage.filename.length > 0
								? sourceImage.filename
								: "image";

						if (
							sourceImage.kind === "remote" &&
						...
						if (
							...
							await imageStorage.save(localId, blob, {
								annotationId:
									typeof sourceAnnotation.id === "string"
										? sourceAnnotation.id
										: `ann-${Date.now()}`,
								filename,
							});
					...
					importedAnnotations.push({
						id:
							typeof sourceAnnotation.id === "string" &&
							sourceAnnotation.id.length > 0
								? sourceAnnotation.id
								: `ann-${Date.now()}-${Math.random().toString(36).slice(2)}`,
						position: asVec3(sourceAnnotation.position, [0, 0, 0]),
						normal: sourceAnnotation.normal
							? asVec3(sourceAnnotation.normal, [0, 0, 1])
							: undefined,
						title:
							typeof sourceAnnotation.title === "string"
								? sourceAnnotation.title
								: "",
						description:
							typeof sourceAnnotation.description === "string"
								? sourceAnnotation.description
								: "",
						images,
						videoUrl:
							typeof sourceAnnotation.videoUrl === "string"
								? sourceAnnotation.videoUrl
								: null,
						links: sanitizeLinks(sourceAnnotation.links),
						color:
							typeof sourceAnnotation.color === "string"
								? sourceAnnotation.color
								: undefined,
						sceneId,
						createdAt:
			...
			importLocalData: async (sceneId) => {
				...
				const sceneSource = sourceAnnotations.filter(
					(annotation) => annotation.sceneId === sceneId,
				);
			...
			migrate: (persistedState: unknown, version: number) => {
				...
				if (version === 0) {
					...
					if (Array.isArray(state.annotations)) {
						state.annotations = (
							state.annotations as Record<string, unknown>[]
						).map((a) => {
				...
				if (version <= 4 && Array.isArray(state.annotations)) {
					state.annotations = (
						...
						const imagesRaw = Array.isArray(annotation.images)
							? annotation.images
							: [];
				...
				if (
					typeof state.sceneMutationVersion !== "object" ||
					state.sceneMutationVersion === null
				) {
					state.sceneMutationVersion = {};
				}
				...
				if (
					typeof state.draftDirtyByScene !== "object" ||
					state.draftDirtyByScene === null
				) {
					state.draftDirtyByScene = {};
				}
				...
				state.officialSceneSyncOverridesByScene =
					normalizeRecoverableSyncOverrides(
						state.officialSceneSyncOverridesByScene as Record<
							string,
							OfficialSceneSyncStatus
						>,
					);
...
if (import.meta.env.DEV)
	(window as unknown as Record<string, unknown>).__store = useViewerStore;

export {
	resolveActiveSceneFromCatalog,
	resolveOfficialSceneSyncDiff,
	resolveOfficialSceneSyncStatusBySceneId,
} from "./sceneCatalog";

export const useActiveScene = () =>
	useViewerStore(resolveActiveSceneFromCatalog);

export const useOfficialSceneSyncDiff = () =>
	useViewerStore(resolveOfficialSceneSyncDiff);
