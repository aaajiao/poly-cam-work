...
function AnnotationEditor({ annotation }: AnnotationEditorProps) {
	const updateAnnotationContent = useViewerStore(
	...
	return (
		<div className="space-y-3 pt-2">
			...
			<div className="border-t border-subtle pt-2">
				<p className="text-faint text-xs uppercase tracking-wide mb-1">
					Description
				</p>
				<textarea
					value={localDesc}
			...
			<div className="border-t border-subtle pt-2">
				...
				<input
					...
					className={cn(
						...
						videoError
							? "border-destructive"
							: "border-subtle focus:border-[color:var(--accent-border)]",
					)}
				/>
				{videoError && (
					<p className="mt-0.5 text-xs text-danger">
						Only Vimeo URLs supported
					</p>
				)}
			</div>
...
export function AnnotationManager() {
	...
	const selectedAnnotation =
		sceneAnnotations.find((a) => a.id === selectedAnnotationId) ?? null;
		...
		<div data-testid="annotation-manager">
				...
				<button
					...
					aria-label={
						annotationsPanelOpen
							? "Collapse annotations panel"
							: "Expand annotations panel"
					}
			...
			{annotationsPanelOpen && (
				<>
					...
					{sceneAnnotations.length > 0 && (
						<div
							className="space-y-1 max-h-48 overflow-y-auto pr-0.5 mt-3"
							data-testid="annotation-list"
						>
							{sceneAnnotations.map((annotation) => {
								const isPanelOpen = openAnnotationPanelIds.includes(
									annotation.id,
								);
								const isSelected =
									annotation.id === selectedAnnotationId || isPanelOpen;
								return (
									<div
										key={annotation.id}
