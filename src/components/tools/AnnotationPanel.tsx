...
function withLineMaterials(
	...
	mutate: (material: LineMaterialMutable) => void,
) {
...
function AnnotationFloatingPanel({
	...
	const lastCameraQuatRef = useRef(
		new THREE.Quaternion().copy(camera.quaternion),
	);
	const lastCameraFovRef = useRef(
	...
	const titleStyle = useMemo(() => {
		...
		if (isLinkHighlighted) {
			style["--title-pulse-duration"] =
				`${linkPulseProfile.titleDuration.toFixed(2)}s`;
			style["--title-pulse-scale"] = linkPulseProfile.titleScale.toFixed(3);
			style["--title-pulse-brightness"] =
				linkPulseProfile.titleBrightness.toFixed(3);
		}
	...
	const startLayoutTransition = useMemo(
		() => () => {
			...
			const pathLength = panelLayoutRef.current.panelPos.distanceTo(
				nextLayout.panelPos,
			);

			transitionIndexRef.current += 1;
			const profile = createTransitionProfile(
				annotation.id,
				transitionIndexRef.current,
				panelCount,
			);

			panelLayoutStartRef.current.panelPos.copy(
				panelLayoutRef.current.panelPos,
			);
			...
			panelTransitionDurationRef.current =
				THREE.MathUtils.clamp(
					PANEL_REPOSITION_BASE_DURATION +
						screenDistance / PANEL_REPOSITION_PIXELS_PER_SECOND,
			...
			const midpoint = panelLayoutRef.current.panelPos
				.clone()
				.lerp(nextLayout.panelPos, 0.5);
			...
			const worldPerPixel =
				(2 * depth * Math.tan(fovRadians * 0.5)) / Math.max(size.height, 1);

			const pathDir = nextLayout.panelPos
				.clone()
				.sub(panelLayoutRef.current.panelPos);
			...
			if (sideAxis.lengthSq() < 0.0001) {
				sideAxis.crossVectors(
					pathDir,
					camera.position.clone().sub(panelLayoutRef.current.panelPos),
				);
			}
			if (sideAxis.lengthSq() < 0.0001) {
				sideAxis.set(1, 0, 0);
			...
			const lateralAmplitude =
				profile.lateralPixels * worldPerPixel * profile.lateralSign;
			const verticalAmplitude =
				lateralAmplitude * profile.verticalFactor * profile.verticalSign;

			transitionLateralOffsetRef.current
				.copy(sideAxis)
				.multiplyScalar(lateralAmplitude);
			transitionVerticalOffsetRef.current
				.copy(upAxis)
				.multiplyScalar(verticalAmplitude);
		},
	...
	useEffect(() => {
		...
		const updateSize = () => {
			...
			if (
				nextWidth !== panelSizeRef.current.width ||
				nextHeight !== panelSizeRef.current.height
			) {
	...
	useFrame((state, delta) => {
		...
		if (isPanelDraggingRef.current) {
			...
			const pulseWave = Math.sin(
				state.clock.elapsedTime * linkPulseProfile.speed +
					linkPulseProfile.phase,
			);
			const pulseFactor = isLinkHighlighted
				? 1 + pulseWave * linkPulseProfile.lineAmplitude
				: 1;
			const glowWave = Math.sin(
				state.clock.elapsedTime * (linkPulseProfile.speed * 1.06) +
					linkPulseProfile.phase +
					1.1,
			);
			const glowPulseFactor = isLinkHighlighted
				? 1 + glowWave * linkPulseProfile.glowAmplitude
				: 1;
		...
		const cameraMoved =
			lastCameraPosRef.current.distanceToSquared(camera.position) >
			CAMERA_POSITION_EPSILON_SQ;
		const cameraRotated =
			1 - Math.abs(lastCameraQuatRef.current.dot(camera.quaternion)) >
			CAMERA_ROTATION_EPSILON;
		const currentFov =
			camera instanceof THREE.PerspectiveCamera ? camera.fov : 50;
		const fovChanged =
			Math.abs(currentFov - lastCameraFovRef.current) > CAMERA_FOV_EPSILON;
		const panelMeasured =
			panelSizeVersionRef.current !== lastPanelSizeVersionRef.current;
		...
		if (panelLayoutTransitionRef.current < 1) {
			if (panelTransitionDelayRef.current > 0) {
				panelTransitionDelayRef.current = Math.max(
					panelTransitionDelayRef.current - delta,
					0,
				);
			} else {
			...
			const panelDiff = panelLayoutRef.current.panelPos.distanceToSquared(
				panelLayoutTargetRef.current.panelPos,
			);
			const midDiff = panelLayoutRef.current.midPos.distanceToSquared(
				panelLayoutTargetRef.current.midPos,
			);
			if (panelDiff > 0.000001 || midDiff > 0.000001) {
				panelLayoutRef.current.panelPos.copy(
					panelLayoutTargetRef.current.panelPos,
				);
...
export function AnnotationPanel() {
	...
	const envelope = useMemo(
		() => getSceneEnvelope(sceneAnnotations),
		[sceneAnnotations],
	);
