...
function loadEnvFile(filePath: string) {
	...
	for (const line of lines) {
		...
		const value =
			quotedDouble || quotedSingle ? rawValue.slice(1, -1) : rawValue;
...
function dedupeModelsById(models: ScanScene[]) {
	...
	for (const model of models) {
		...
		const nextUpdatedAt =
			typeof model.updatedAt === "number" ? model.updatedAt : 0;
...
function normalizeRegistryModels(
	raw: ModelRegistryDocument | ScanScene[] | null,
): ScanScene[] {
	if (!raw) return [];
...
async function collectReferencedImagePathnames(
	sceneIds: string[],
	releasePaths: string[],
) {
	...
	for (const sceneId of sceneIds) {
		const draft = await readJsonBlob<SceneDraft>(
...
async function main() {
	...
	loadLocalEnvFiles();
	if (!readBlobReadWriteToken()) {
		throw new Error(
			`${BLOB_READ_WRITE_TOKEN_ENV_NAME} is missing. Set it in environment or .env.local before running cleanup. ${LEGACY_BLOB_READ_WRITE_TOKEN_ENV_NAME} is still accepted as a legacy fallback.`,
		);
	}
	...
	const modelRegistryRaw = await readJsonBlob<
		ModelRegistryDocument | ScanScene[]
	>("models/index.json");
	...
	const modelRegistryUpdated =
		modelRegistryRaw !== null &&
		JSON.stringify(normalizedModels) !==
			JSON.stringify(
				Array.isArray(modelRegistryRaw)
					? modelRegistryRaw
					: (modelRegistryRaw.models ?? []),
			);
						...
						normalizedModels
							...
							.filter(
								(pathname): pathname is string =>
									pathname !== null && isManagedModelAssetPathname(pathname),
							),
					);

					const existingModelPathnames = allPathnames.filter((pathname) =>
						isManagedModelAssetPathname(pathname),
					);
	...
	const referencedImagePathnames = await collectReferencedImagePathnames(
		sceneIds,
		releasePaths,
	);
	const existingSceneImagePathnames = allPathnames.filter((pathname) =>
		isSceneImagePathname(pathname),
	);
	const staleSceneImagePathnames = existingSceneImagePathnames
		.filter((pathname) => !referencedImagePathnames.has(pathname))
		.sort((a, b) => a.localeCompare(b));
	const legacyRootImagePathnames = allPathnames.filter((pathname) =>
		isLegacyRootImagePathname(pathname),
	);
	const staleLegacyImagePathnames = legacyRootImagePathnames
		.filter((pathname) => !referencedImagePathnames.has(pathname))
		.sort((a, b) => a.localeCompare(b));
	const staleImagePathnames = Array.from(
		new Set([...staleSceneImagePathnames, ...staleLegacyImagePathnames]),
	);
