...
function parseClientPayload(payload: string | null | undefined): { sceneId: string; annotationId: string } {
  if (!payload) {
    throw new Error('Missing upload metadata')
  }

  const parsed = JSON.parse(payload) as MediaUploadPayload
  const sceneId = typeof parsed.sceneId === 'string' ? sanitizeSegment(parsed.sceneId) : ''
  const annotationId = typeof parsed.annotationId === 'string' ? sanitizeSegment(parsed.annotationId) : ''
