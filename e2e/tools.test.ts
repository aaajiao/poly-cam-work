...
test.describe('Tool Modes', () => {
  ...
  test('clicking clip tool activates it', async ({ page }) => {
    await page.click('[data-testid="tool-clip"]')
    await expect(page.locator('[data-testid="tool-clip"]')).toHaveClass(/bg-blue-600/)
  })
  ...
  test('keyboard shortcut C activates clip tool', async ({ page }) => {
  ...
  test('clip controls appear in sidebar when clip tool active', async ({ page }) => {
    await page.click('[data-testid="tool-clip"]')
